...
class BillingManager(context: Context) : PurchasesUpdatedListener {
    companion object {
        const val PRO_PRODUCT_ID = "de.kamilunavo.brennercalc.pro"
    }
    ...
    fun launchPurchase(activity: Activity) {
        ...
        if (details == null) {
            statusMessage = "BrennerCalc Pro is not available yet."
            queryProduct()
            return
        }
    ...
    private fun queryProduct() {
        ...
        billingClient.queryProductDetailsAsync(params) { billingResult, result ->
            if (billingResult.responseCode != BillingClient.BillingResponseCode.OK) {
                statusMessage = billingResult.debugMessage.ifBlank { "Could not load BrennerCalc Pro." }
    ...
    private fun processPurchases(purchases: List<Purchase>) {
        ...
        isPro = ownedPurchase != null

        ownedPurchase?.let { purchase ->
